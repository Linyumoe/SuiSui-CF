...
func setupTestDB(t *testing.T) {
	...
	db, err = sql.Open("sqlite", "file::memory:") // each test gets its own DB
	if err != nil {
		t.Fatal(err)
	}
...
// resetTables truncates test data so each test starts with a clean slate.
func resetTables() {
...
func TestNotesCRUD(t *testing.T) {
	...
	if err := json.Unmarshal(w3.Body.Bytes(), &notes); err != nil {
		t.Fatalf("failed to unmarshal notes list: %v", err)
	}
	if len(notes) != 1 {
...
func TestNotesPagination(t *testing.T) {
	...
	if err := json.Unmarshal(w2.Body.Bytes(), &pResp); err != nil {
