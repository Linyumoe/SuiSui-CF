...
func handleNotes(w http.ResponseWriter, r *http.Request, path string) {
	...
	case path == "/notes" && r.Method == "GET":
		...
		offset := 0
		if l := q.Get("limit"); l != "" {
			if n, err := fmt.Sscanf(l, "%d", &limit); n != 1 || limit < 0 {
				if err != nil {
					log.Printf("failed to parse notes limit %q: %v", l, err)
				}
				limit = 0
			}
			if limit > 200 {
		...
		if o := q.Get("offset"); o != "" {
			if n, err := fmt.Sscanf(o, "%d", &offset); n != 1 || offset < 0 {
				if err != nil {
					log.Printf("failed to parse notes offset %q: %v", o, err)
				}
				offset = 0
			}
		}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&id, &content, &createdAt, &updatedAt, &pinned, &tags, &username, &avatar, &nickname); err != nil {
				log.Printf("scan error in handleNotes list: %v", err)
				continue
			}
	...
	case path == "/notes" && r.Method == "POST":
		...
		if n.Username == "" || n.Username != requester {
			errResp(w, "unauthorized", 401)
			return
		}
	...
	case strings.Contains(path, "/upload") && r.Method == "POST":
		...
		url, err := saveUploadedFile(w, r, "image")
		if err != nil {
	...
	case strings.HasSuffix(path, "/react") && (r.Method == "POST" || r.Method == "DELETE"):
		...
		if !strings.HasPrefix(body.Username, "guest_") && owner != body.Username {
	...
	case path == "/notes/reorder" && r.Method == "PATCH":
		...
		tx, err := db.Begin()
		if err != nil {
			errResp(w, "failed to start transaction", 500)
			return
		}
		defer tx.Rollback()
		for i, id := range body.Order {
			if _, err := tx.Exec("UPDATE notes SET pin_order=? WHERE id=? AND username=?", i, id, requester); err != nil {
				errResp(w, "重排失败", 500)
				return
			}
		}
	...
	default:
		...
		if len(parts) == 1 && r.Method == "PUT" {
			...
			var callerRole string
			if err := db.QueryRow("SELECT role FROM users WHERE username=?", requester).Scan(&callerRole); err != nil {
				log.Printf("failed to query caller role: %v", err)
			}
			if requester != owner && callerRole != "admin" {
				errResp(w, "forbidden", 403)
				return
			}
			...
			if _, err := db.Exec("UPDATE notes SET pinned = CASE WHEN pinned=0 THEN 1 ELSE 0 END WHERE id=? AND username=?", parts[0], requester); err != nil {
				errResp(w, "操作失败", 500)
				return
			}
			sseBroadcast("note", "updated")
...
func handleTrash(w http.ResponseWriter, r *http.Request, path string) {
	...
	if r.Method == "GET" && path == "/notes/trash" {
		...
		if l := r.URL.Query().Get("limit"); l != "" {
			if _, err := fmt.Sscanf(l, "%d", &limit); err != nil {
				log.Printf("failed to parse trash limit %q: %v", l, err)
			}
		}
		if o := r.URL.Query().Get("offset"); o != "" {
			if _, err := fmt.Sscanf(o, "%d", &offset); err != nil {
				log.Printf("failed to parse trash offset %q: %v", o, err)
			}
		}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&id, &content, &createdAt, &updatedAt, &pinned, &tags, &uname, &avatar, &nickname, &deletedAt); err != nil {
				log.Printf("scan error in handleTrash: %v", err)
				continue
			}
			var tagList []string
			if err := json.Unmarshal([]byte(tags), &tagList); err != nil {
	...
	if r.Method == "PATCH" && strings.HasSuffix(path, "/restore") {
		...
		res, err := db.Exec("INSERT OR IGNORE INTO notes (id, content, created_at, updated_at, pinned, tags, username, avatar, nickname) VALUES (?,?,?,?,?,?,?,?,?)", noteId, cont, ct, ut, p, ts, username, av, nk)
		if err != nil {
			errResp(w, "恢复失败", 500)
			return
		}
...
func batchGetReactions(ids []string) map[string]map[string][]string {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &emoji, &username); err != nil {
				log.Printf("scan error in batchGetReactions: %v", err)
				continue
			}
		if result[id] == nil {
...
func handleNotesExport(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &content, &createdAt, &updatedAt, &pinned, &tags, &uname, &avatar, &nickname); err != nil {
			log.Printf("scan error in handleNotesExport: %v", err)
			continue
		}
		var tagList []string
		if err := json.Unmarshal([]byte(tags), &tagList); err != nil {
			log.Printf("failed to parse tags in export note %s: %v", id, err)
		}
		notes = append(notes, noteResponse{
			ID: id, Content: content, CreatedAt: createdAt, UpdatedAt: updatedAt,
			Pinned: pinned == 1, Tags: tagList, Username: uname,
	...
	// Batch-load reactions for exported notes
	var allIds []string
	for _, n := range notes {
...
func handleNotesImport(w http.ResponseWriter, r *http.Request) {
	...
	for _, n := range notes {
		...
		tagBytes, err := json.Marshal(n.Tags)
		if err != nil {
			log.Printf("failed to marshal tags for note %s: %v", n.Id, err)
			continue
		}
		pinned := 0
		if n.Pinned {
		...
		res, err := db.Exec("INSERT OR IGNORE INTO notes (id, content, created_at, updated_at, pinned, tags, username, avatar, nickname) VALUES (?,?,?,?,?,?,?,?,?)",
			n.Id, n.Content, n.CreatedAt, n.UpdatedAt, pinned, string(tagBytes), n.Username, n.Avatar, n.Nickname)
		if err != nil {
			log.Printf("failed to import note %s: %v", n.Id, err)
			continue
		}
...
func handleShareView(w http.ResponseWriter, r *http.Request) {
	...
	var tagList []string
	if err := json.Unmarshal([]byte(tags), &tagList); err != nil {
		log.Printf("failed to parse tags in share view for note %s: %v", noteId, err)
	}
