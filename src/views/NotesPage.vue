...
import { useNotePolling } from "@/composables/useNotePolling"

const showMobileHeatmap = inject("showMobileHeatmap", ref(false))
...
const { newNotesCount, onNoteSubmitted, startPolling, refreshNotes } = useNotePolling()
...
async function loadSiteIcp() {
  ...
  } catch (err) { console.error("[NotesPage] loadSiteIcp failed:", err) }
}
...
async function fetchVersion() {
  ...
  } catch (err) { console.error("[NotesPage] fetchVersion failed:", err); versionText.value = "" }
}
...
<template>
  <div class="notes-layout" :class="{ mobile: isMobile }">
    ...
    <div class="main-col">
      <v-dialog v-model="showMobileHeatmap" max-width="400" scrollable persistent transition="dialog-bottom-transition">
        <v-card class="rounded-xl pa-4 heatmap-dialog-card">
          <div class="d-flex align-center mb-3">
            <span class="text-subtitle-2 font-weight-medium">活动日历</span>
            <v-spacer /><v-btn icon="mdi-close" size="small" variant="text" @click="showMobileHeatmap = false" />
          </div>
          ...
          <Heatmap class="mb-4" @select-day="selectedDay = $event; showMobileHeatmap = false" />
          <v-card variant="outlined" class="rounded-xl pa-4 side-card">
            ...
            <div class="d-flex flex-wrap ga-1">
              <v-chip v-for="[tag] in store.allTags" :key="tag" size="x-small" class="tag-chip"
                :color="tagColor(tag)"
                :variant="selectedTag === tag ? 'flat' : 'outlined'" @click="selectedTag = selectedTag === tag ? '' : tag; showMobileHeatmap = false">
                #{{ tag }}
              </v-chip>
      ...
      <div v-if="!store.loaded" class="d-flex flex-column ga-3 px-1">
        <div v-for="i in 5" :key="i" class="skeleton-card">
          ...
          <div class="skeleton-row" style="width:75%" />
          <div class="skeleton-row" style="width:55%" />
        </div>
      </div>
      <template v-else>
...
.heatmap-dialog-card { background: rgba(var(--v-theme-surface), 0.92) !important; backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); }
