...
function copyContent() {
  navigator.clipboard.writeText(props.memo.content).then(() => {
    ...
    console.error("[NoteCard] clipboard.writeText failed, using fallback:", err)
    const ta = document.createElement("textarea")
    ta.value = props.memo.content
    document.body.appendChild(ta)
    ta.select()
    try { document.execCommand("copy") } catch (e) { console.error("[NoteCard] execCommand copy fallback also failed:", e) }
