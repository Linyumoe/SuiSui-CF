...
async function fetchGitHubRepos() {
  ...
  for (const fullName of repos) {
    ...
    try {
      const res = await fetch(API_BASE_URL + `/api/gh/repos/${fullName}`)
    ...
    } catch (e) { console.error("[MarkdownPreview] fetchGitHubRepos failed:", e) }
  }
}
...
const rendered = computed(() => {
  ...
  try {
    ...
    return html
  } catch (err) { console.error("[MarkdownPreview] rendering failed:", err); return props.content }
})
...
function handleClick(e: MouseEvent) {
  ...
  function doCopy(text: string) {
    ...
    } catch { console.error("[MarkdownPreview] clipboard API not supported") }
    const ta = document.createElement("textarea")
    ta.value = text; ta.style.position = "fixed"; ta.style.opacity = "0"
    document.body.appendChild(ta); ta.select()
    try { document.execCommand("copy") } catch { console.error("[MarkdownPreview] execCommand copy fallback failed") }
    document.body.removeChild(ta)
    return Promise.resolve()
  }
  const codeKey = code.slice(0, 32)
  copyStatus.value[codeKey] = "copied"
  doCopy(code).then(() => {
    copyBtn.classList.add("copied")
    setTimeout(() => {
      copyStatus.value[codeKey] = "idle"
      copyBtn.classList.remove("copied")
    }, 2000)
  }).catch(() => {
    copyStatus.value[codeKey] = "error"
